...
check('uncommitted card shows the dirt', (await page.locator('.git-row').first().textContent())?.includes('Uncommitted Changes (1)'));
...
check('uncommitted card gone', (await page.locator('.git-row', { hasText: 'Uncommitted Changes' }).count()) === 0);
check('stash row present', (await page.locator('.git-row:has(.git-ref-stash)').count()) === 1);

// 2. Open the stash detail: actions + the stashed diff.
await page.locator('.git-row:has(.git-ref-stash)').first().click();
await page.waitForTimeout(900);
check('detail titled with the entry', (await page.locator('.git-detail-title').first().textContent())?.includes('WIP on '));
const actionLabels = await page.locator('.git-detail .git-bulk .pkgs-btn').allTextContents();
check('Apply/Pop/Drop actions', actionLabels.join(',') === 'Apply,Pop,Drop');
check('stash diff renders', (await page.locator('.git-detail .git-diff .dl').count()) > 0);

// 3. Apply keeps the entry and brings the dirt back.
await page.click('.git-detail .pkgs-btn:text-is("Apply")');
await page.waitForTimeout(1500);
...
await page.locator('.git-row:has(.git-ref-stash)').first().click();
await page.waitForTimeout(600);
const drop = page.locator('.git-detail .pkgs-btn.git-danger');
await drop.click();
check('drop armed, not run', stashCount() === 1);
await page.locator('.git-detail .pkgs-btn.git-danger[data-armed="1"]').click();
await page.waitForTimeout(1500);
...
await page.locator('.git-row:has(.git-ref-stash)').first().click({ button: 'right' });
